...
function emitToParent(event, payload) {
    if (window.parent !== window) {
        // Same-origin host (Next shell serves /game) → target its origin explicitly
        window.parent.postMessage({ type: event, data: payload }, window.location.origin);
    }
}
...
window.addEventListener('message', (e) => {
    ...
    if (msg.type === 'host:config') {
        GAME_SESSION.wallet = msg.data?.wallet;
        GAME_SESSION.season = msg.data?.season;
        GAME_SESSION.skin = msg.data?.skin;
    } else if (msg.type === 'host:newRun') {
        GAME_SESSION.runId = msg.data?.runId;
        GAME_SESSION.mode = msg.data?.mode || 'survival';
    }
});

// Emit game:ready when script loads
...
function resetGame(mode = "survival") {
    ...
    // Emit game:start to parent
    if (GAME_SESSION.runId) {
        GAME_SESSION.seed = Math.floor(Math.random() * 2147483647);
        GAME_SESSION.startTime = performance.now();
        GAME_SESSION.eventLog = [];
        emitToParent('game:start', {
            runId: GAME_SESSION.runId,
...
function updateScoreUI() {
    ...
    // Emit game:score every 5s to parent
    if (GAME_SESSION.runId && STATE.isRunning) {
        const now = performance.now();
        if (!GAME_SESSION.lastScoreEmit || (now - GAME_SESSION.lastScoreEmit) >= 5000) {
            GAME_SESSION.lastScoreEmit = now;
            emitToParent('game:score', {
                runId: GAME_SESSION.runId,
                score: STATE.score.total,
                elapsedMs: Math.round((now - GAME_SESSION.startTime || 0) + STATE.elapsedGameTime * 1000)
            });
        }
    }
}
...
function animate(time) {
    ...
    if (
        ...
        // Emit game:over to parent
        if (GAME_SESSION.runId) {
            const elapsedMs = Math.round((performance.now() - (GAME_SESSION.startTime || 0)) + STATE.elapsedGameTime * 1000);
            emitToParent('game:over', {
                runId: GAME_SESSION.runId,
                seed: GAME_SESSION.seed,
                score: STATE.score.total,
                elapsedMs: elapsedMs,
                ...
                money: STATE.money,
                eventLog: GAME_SESSION.eventLog
            });
        }
